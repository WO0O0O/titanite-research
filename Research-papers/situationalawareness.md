...
But by counting the OOMs, we get a peek at
...
~ elementary schooler: “Wow, with just some few-
...
5
and college aptitude tests (Figure 7).

Or consider the MATH benchmark, a set of difficult mathemat-
6
ics problems from high-school math competitions.
When the
...
7
imal progress over the coming years (Figure 8);
and yet within
...
8 If there’s one lesson we’ve learned from the past
proven wrong.
...
compared to in-domain PhDs who get
...
If we can count the OOMs, we can (roughly, qualita-

With each OOM of effective compute, models predictably, reliably get
better.10
tively) extrapolate capability improvements.
...
Estimated Compute Growth

GPT-2 (2019) ~4e21 FLOP
GPT-3 (2020) ~3e23 FLOP
GPT-4 (2023)
...
3.75-level” and GPT-4-level performance,
while costing
...
data, training stack, etc. all the time.
...
on average ~0.5 OOMs/year of compute effi-
to continue:
...
rapidly.
(The publicly-inferable inference cost efficiencies,
at least, don’t seem to have slowed down at all.) On the high
21
end, we could even see more fundamental, Transformer-like
...
possible—is useful here as well.
...
els have incredible latent capabilities,
...
More generally, context is key to unlock-
efficiency gain.
...
to write an essay via their initial stream-of-consciousness.
...
that lets it reason through difficult, long-
...
level model in a minute.30

The OOM effective compute scaleup
...
gap we’ll cover once more, where will that take us?
...
We’d also want to reserve some of the GPUs for
...
tokens, every single day.37
In any case, the exact numbers don’t
...
released GPT-4,
merely a year later, while providing similar
...
I think just about every researcher at

tomated Alec Radfords.40
...
if we got 5 OOMs in the last decade, we should
limits,
...
Or you could “spend”
for each than labs currently do.
...
) But something like 5 OOMs seems very
...
sizeable (many OOMs) one-time gain.
...
would make extraordinary advances in many fields
...
covering all of the Nevada desert
...
Earlier, we found a roughly ~0.5 OOMs
/year trend growth of
...
large fraction of GPUs will probably be be used for inference
...
than the chips (site, building, cooling, power, etc.).
...
bers: Microsoft and Google will likely do $50B+
, AWS and
...
training clusters, let’s say 2x/year.
...
many-hundred-billion dollar corporate bond sale then.
...
cluster by the end of the decade seems on the table.
...
Each rig can drill roughly 3 wells per month, so 40
ter.
rigs (the current rig count in the Marcellus) could build
60
up the production base for 100GW in less than a year.
...
power multiple trillion-dollar datacenters.
...
for 100GW of natural gas power
about $100B of capex
...
one datacenter).
Of course, not all of TSMC will be able to be
...
in the past 5 years; they’d likely need to
AI. TSMC ~doubled
...
risks the AGI weights getting stolen
(and perhaps be shipped
...
wakeup.”
Behind the scenes, the most staggering techno-
...
They admit to being at level 0
most capable state actors).
...
just look through office windows. And
any point.
...
the same intense security!

75

• Novel technical advances on confidential compute / hard-
and extreme scrutiny on the entire hard-
...
the intelligence explosion and post-superintelligence.
...
dangerous, reckless, and unpredictable actors.
...
in fission matters.”
But he was rebuffed by most; secrecy was
...
be worth hundreds of billions of dollars to the CCP.
The real-
...
Misaligned superintelligence is prob-
I am not a doomer.
ably not the biggest AI risk.
But I did spend the past year
...
plan for how we’ll muddle through,
...
Base models had lots of raw smarts but weren’t
...
penalize bad behavior with RLHF.
...
than “default plan for muddling through” bucket.
...
ROME is able to
in models with only unsupervised data.
...
thing like an “AI lie detector”
—and without requiring funda-
...
across the board, from hardware encryption to
exfiltration
And once our AGIs can do automated
many-key signoff.
...
picking up some abstruse malign behavior.
Moreover, while
...
97
to align “somewhat-superhuman” systems will mostly work.
...
and a critical open
rity of Chinese abilities here is debated,
question is in what quantities they could produce these 7nm
chips.
Still, it seems like there’s at least a very reasonable
...
). 7nm vs. 3nm or 2nm, and their general fab imma-
105
...
chain
turity, probably makes things more expensive for China.
...
training clusters in a few years.
...
At the very least, the odds we get something
tage, too great.
...
have a healthy lead, say 2 years.
That buys us the time nec-
...
the Cold War in one piece involved too much luck
...
whatever (though it might!).
Rather, I expect a more suave
...
trillions of investment involved, and for checks-and-balances.
...
to literally coup the US government.
Imagine if Elon Musk
had final command of the nuclear arsenal.
...
cratic governments should control the military;
...
But once it becomes clear that superintel-
nuclear arsenals!).
...
Almost all other AI lab and AI governance politicking is
